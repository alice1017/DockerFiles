...
# ${WINDOW:+"[$WINDOW]"} = sreen 実行時にスクリーン番号を表示 (prompt_subst が必要)
...
# alias to grep extention
alias -g html="| grep '\.html$'"
alias -g py="| grep '\.py$'"
alias -g dot="| grep '^\.'"
...
# branch
#PROMPT="%{$fg[blue]%}($(git branch -a 2>/dev/null | grep "^*" | tr -d '\* '))%{$reset_color%}$PROMPT"
# 名前@マシン名 プロンプト
